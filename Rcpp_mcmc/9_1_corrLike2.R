rhierMnlRwMixture_inLike_X0_mcprice=
  function(Data,Prior,Mcmc)
  {
    ...
    #             flag_know_state is an indicator of weither we know initial state
    #               it is assigned only for i = 1 as it is the same for all i
    #             count_out is a # of observation in which person i forms his state first
    #               time in the data
    #             initial_price_id is state of price in observation 1 for each individual
    ...
    nlgt=length(lgtdata)
    drawdelta=TRUE
    if(is.null(Data$Z)) { cat("Z not specified",fill=TRUE);   drawdelta=FALSE}
    ...
    ######################
    ### August 2017 -- inputs for markovian prices case
    transition_matrix_median_steps <- lgtdata[[1]]$transition_matrix_median_steps
    price_transition_states <- lgtdata[[1]]$price_transition_states
    vec_price_states_probs <- lgtdata[[1]]$vec_price_states_probs
    draws_length <- lgtdata[[1]]$draws_length
    number_price_simulations <- lgtdata[[1]]$number_price_simulations
    flag_markovian <- lgtdata[[1]]$flag_markovian
    flag_know_state <- lgtdata[[1]]$flag_know_state # if TRUE, inputs for all units should include s1 and initial_price_state_dropped
    #####################
    ...
    mnlRwMetropOnce=
      function(y,X,count_out,initial_price_id,initial_price_state_dropped,s1,transition_matrix_median_steps,price_transition_states,vec_price_states_probs,draws_length,number_price_simulations,flag_markovian, flag_know_state, oldbeta,oldll,s,inc.root,betabar,rootpi){ 
        #
        ...
        cll=llmnl_initial_mcprice(betac,y,X,count_out,initial_price_id,initial_price_state_dropped,s1,transition_matrix_median_steps,price_transition_states,vec_price_states_probs,draws_length,number_price_simulations,flag_markovian, flag_know_state)
        clpost=cll+lndMvn(betac,betabar,rootpi)
        ldiff=clpost-oldll-lndMvn(oldbeta,betabar,rootpi)
        ldiff=Re(ldiff) # fixing a strange case of complex numbers (0i in some numbers,so doesn't change values)
    ...
    for(rep in 1:R)
    {
      ...
      for (lgt in 1:nlgt) 
      {
        ...
        if (rep == 1) 
        { oldll[lgt]=llmnl_initial_mcprice(oldbetas[lgt,],lgtdata[[lgt]]$y,lgtdata[[lgt]]$X, lgtdata[[lgt]]$count_out, lgtdata[[lgt]]$initial_price_id, lgtdata[[lgt]]$initial_price_state_dropped, lgtdata[[lgt]]$s1, transition_matrix_median_steps, price_transition_states, vec_price_states_probs, draws_length, number_price_simulations, flag_markovian, flag_know_state)}  
        #   compute inc.root
        inc.root=chol(chol2inv(chol(lgtdata[[lgt]]$hess+rootpi%*%t(rootpi))))
        metropout=mnlRwMetropOnce(lgtdata[[lgt]]$y,lgtdata[[lgt]]$X,lgtdata[[lgt]]$count_out, lgtdata[[lgt]]$initial_price_id, lgtdata[[lgt]]$initial_price_state_dropped, lgtdata[[lgt]]$s1, transition_matrix_median_steps, price_transition_states, vec_price_states_probs, draws_length, number_price_simulations, flag_markovian, flag_know_state, oldbetas[lgt,],
